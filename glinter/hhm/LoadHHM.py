# modified by zw2x from RaptorX
import os
import sys
import numpy as np
import pickle

from glinter.hhm.SequenceUtils import AALetter2OrderOf1LetterCode, AA1LetterOrder23LetterOrder, DetectMultiHIS
"""
...
def load_hhm(hhmfile):
    ...
    while i < len(content):
        ...
        if row.startswith('>' + protein['name']) and (content[i-1].strip() != 'SEQ') and ('sequence' not in protein):
...
if __name__ == "__main__":
    ...
    savefile = os.path.dirname(file) + '/' + os.path.basename(file) + '.pkl'
